...
static inline void pio_spi_psram_cs_init(PIO pio, uint sm, uint prog_offs, uint n_bits, uint16_t clkdiv_int, uint8_t clkdiv_frac, bool fudge, uint pin_cs, uint pin_mosi, uint pin_miso) {
    pio_sm_config c;
    if (fudge) {
...
#include "hardware/gpio.h"
static inline void pio_qspi_psram_cs_init(PIO pio, uint sm, uint prog_offs, uint n_bits, uint16_t clkdiv_int, uint8_t clkdiv_frac, uint pin_cs, uint pin_sio0) {
